...
//!
//! Feeds arbitrary bytes through both `JsonProcessor::process` (re-serialize
...
//! or adversarial input — the edit path does byte-span slicing on untrusted
//! parser output, so it is the higher-risk path.
...
fuzz_target!(|data: &[u8]| {
    ...
    // Span-based edit path (parse → span edits → apply_edits).
